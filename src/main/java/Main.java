...
public class Main {
	...
	//Pre-2014 way
	private static void unsafeThreadedDepositOld() throws InterruptedException {
		...
		Runnable unsafeRunnable1 = new Runnable() {
			@Override
			public void run() {
				b2.DepositCash();
		...
		Runnable unsafeRunnable2 = new Runnable() {
			@Override
			public void run() {
				b2.DepositCash();
		...
		System.out.println("Two threads unlocked (old way)");
		b2.PrintBalance();
	}
